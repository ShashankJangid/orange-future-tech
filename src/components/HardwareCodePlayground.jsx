    ...
    <section id="playground" class="py-20 relative z-10 font-['Plus_Jakarta_Sans',sans-serif]">
      <div class="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <motion.div
          ...
          class={`rounded-3xl border overflow-hidden shadow-2xl transition-all duration-300 ${
        ...
        >
          <div class={`px-6 py-4 border-b flex flex-wrap items-center justify-between gap-3 ${
            darkMode ? 'bg-[#0B0F17] border-slate-800' : 'bg-slate-100/90 border-slate-200'
          }`}>
          ...
          <div class={`p-6 sm:p-8 ${darkMode ? 'bg-[#080B11]' : 'bg-white'}`}>
            {activeMode === 'code' ? (
              <motion.div
              ...
              >
                <div class={`lg:col-span-7 font-mono-code text-xs overflow-x-auto leading-relaxed p-4 rounded-2xl border ${
              ...
              >
                ...
                <div class={`relative w-full rounded-3xl border p-6 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden ${
                  darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'
                }`}>
                  <div class={`flex-1 w-full flex items-center justify-center p-4 rounded-2xl border shadow-inner overflow-hidden max-h-60 ${
                    darkMode ? 'bg-[#0B0F17] border-slate-800' : 'bg-slate-900 border-slate-800'
                  }`}>
                    <img
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      <div class="flex items-center gap-2">
                        ...
                        ></span>
                        <span class={`text-xs font-mono-code uppercase font-semibold ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                          {currentComp.type}
                        </span>
                      </div>

                      <h4 class={`text-lg font-bold font-['Orbitron',sans-serif] ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                        {currentComp.name}
                      </h4>

                      <p class={`text-xs leading-relaxed font-normal ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                        {currentComp.details}
                      </p>

                      <div class="grid grid-cols-2 gap-2.5 pt-2 text-[11px] font-mono-code">
                        <div class={`p-2.5 rounded-xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                          <span class="text-slate-500 block">FOOTPRINT</span>
                          <span class={`font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>{currentComp.pins}</span>
                        </div>
                        <div class={`p-2.5 rounded-xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                          <span class="text-slate-500 block">VOLTAGE / BUS</span>
                          <span class={`font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>{currentComp.voltage}</span>
                        </div>
                      </div>
                    </motion.div>
                  </AnimatePresence>
